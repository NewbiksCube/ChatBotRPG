import os
import json
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QTextEdit, QLabel, QHBoxLayout, QPushButton, QScrollArea, QFrame, QSizePolicy
...
class NotesManagerWidget(QWidget):
    ...
    def __init__(self, theme_colors, tab_settings_file, parent=None):
        ...
        self.world_file = os.path.join(os.path.dirname(tab_settings_file), "resources", "data files", "worldnotes.json")
        self.system_file = os.path.join(os.path.dirname(tab_settings_file), "resources", "data files", "systemnotes.json")
    ...
    def _setup_default_world_fields(self):
        ...
        for field_key in default_fields:
            ...
            if field_key not in self.world_field_instances:
                ...
                self.world_field_instances[field_key][0] = {
                    'custom_name': None,
                    'content': ''
                }
    ...
    def add_field_instance(self, field_type, custom_name=None):
        ...
        if instances:
            instance_id = max(int(k) for k in instances.keys()) + 1
        else:
            instance_id = 0
        instance_data = {
    ...
    def move_instance_left(self, field_type, instance_id):
        if field_type in self.world_field_instances:
            instances = self.world_field_instances[field_type]
            instance_ids = sorted(instances.keys(), key=int)
    ...
    def save_notes(self):
        if self._notes_changed_since_last_save:
            try:
                tab_settings = {}
                ...
                world_data = {
                    'world_field_instances': self.world_field_instances,
                    'world_field_order': self.world_field_order
                }
                ...
                for field_key, text_field in self.system_fields.items():
                    ...
                    system_data[f'system_{field_key}'] = content
                os.makedirs(os.path.dirname(self.system_file), exist_ok=True)
        ...
        try:
            ...
            if os.path.exists(self.system_file):
                ...
                if isinstance(system_data, dict):
                    for field_key, text_field in self.system_fields.items():
                        system_key = f'system_{field_key}'
                        if system_key in system_data:
                            text_field.blockSignals(True)
                            text_field.setPlainText(system_data[system_key])
                            text_field.blockSignals(False)
            ...
            if os.path.exists(self.world_file):
                ...
                if isinstance(world_data, dict):
                    if 'world_field_instances' in world_data:
                        loaded_instances = world_data['world_field_instances']
                        self.world_field_instances = {}
                        for field_type, instances in loaded_instances.items():
                            self.world_field_instances[field_type] = {}
                            for instance_id, instance_data in instances.items():
                                self.world_field_instances[field_type][int(instance_id)] = instance_data
                    if 'world_field_order' in world_data:
                        self.world_field_order = world_data['world_field_order']
            if not self.world_field_instances:
                self._setup_default_world_fields()
